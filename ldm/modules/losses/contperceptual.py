...
class LPIPSWithDiscriminatorTPL(nn.Module):
    def __init__(self, disc_start, logvar_init=0.0, kl_weight=1.0, pixelloss_weight=1.0,
                 disc_num_layers=3, disc_in_channels=3, disc_factor=1.0, disc_weight=1.0,
                 perceptual_weight=1.0, text_perceptual_weight=1.0, use_actnorm=False, 
        ...
        self.pixel_weight = pixelloss_weight
        self.perceptual_loss = LPIPS().eval()
        self.text_perceptual_loss = TextPerceptualLoss().eval()
        self.text_perceptual_weight = text_perceptual_weight
        self.perceptual_weight = perceptual_weight
        # output log variance
        self.logvar = nn.Parameter(torch.ones(size=()) * logvar_init)
    ...
    def forward(self, inputs, reconstructions, posteriors, optimizer_idx,
                global_step, bboxes, last_layer=None, cond=None, split="train",
                weights=None):
        rec_loss = torch.abs(inputs.contiguous() - reconstructions.contiguous())
        if self.perceptual_weight > 0:
            p_loss = self.perceptual_loss(inputs.contiguous(), reconstructions.contiguous())
